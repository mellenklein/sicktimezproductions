<?php
$model = new SortModel('distributor_downloads');
$items = $model->get_items(array('order' => 'sort'));
$item = array();
$item_names = array(
	'singular'=>'Distributor Download Item',
	'plural'=>'Distributor Download Items'
);
$view = 'list';
...
if($action == 'update'){
	...
	if(!empty($_FILES["download_file"]['name'])) {
		$fields['file'] = upload_file('./../assets/distributor_downloads/', 'download_file');
	}
	
	if(!empty($model->data['id'])){
		if(!empty($model->data['file']) && isset($fields['file'])) {
			unlink(dirname ( __FILE__ ).'/../../assets/distributor_downloads/'.$model->data['file']);
		}
...
$(function() {
	$('#remove-file').click(function(evt){
		evt.preventDefault();
		var file = '<?php echo dirname ( __FILE__ ).'/../../assets/distributor_downloads/'; ?>'+$('#cur_file').attr('data-file');
		if (window.confirm("Do you really want to delete this file?")) {
			$.post('<?php echo $config->get('admin_url'); ?>ajax', {action:'delImage',file:file, field:'file', table:'distributor_downloads', id:<?php echo json_encode($id); ?>}, function(data){
				if(data > 0){
					$('#cur_file').remove();
				}
			});
...
<table class="cms_table">
  ...
  <tbody class="sortable" rel="distributor-downloads">
	<?php foreach($items as $i): ?>
	
	<tr id="sort_<?php echo $i['id']; ?>">
		<td class="sorter"><img src="<?php echo $config->get('admin_url'); ?>images/sort.png" alt="sort" /></td>
		<td><?php echo $i['title']; ?></td>
		<td><a href="<?php echo $config->get('url').'assets/distributor_downloads/'.$i['file']; ?>"><?php echo $i['file']; ?></a></td>
		<td align="center">
